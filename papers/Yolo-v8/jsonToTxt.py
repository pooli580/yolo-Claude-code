...
def labelme2yolov2Seg(jsonfilePath="", resultDirPath="", classList=["0", "1", "2", "3", "4"]):
    """
    ...
    for jsonfile in jsonfileList:
        ...
        with open(jsonfile, "r",encoding='gbk') as f:
            try:
                file_in = json.load(f,strict=False)
            except:
                unload.append(jsonfile)
                print('这个文件没有被加载进来，解析失败',jsonfile)
            print(unload)
            print(len(unload))
...
if __name__ == "__main__":
    jsonfilePath = "/nvme1/liuyundong/xianyinggunYoloV8/datasets/json"  # 要转换的json文件所在目录
    resultDirPath = "/nvme1/liuyundong/xianyinggunYoloV8/datasets/labels/train20230304"  # 要生成的txt文件夹
    # jsonfilePath = "/nvme1/liuyundong/xianyinggunYoloV8/huashangdatasets/json"  # 要转换的json文件所在目录
    # resultDirPath = "/nvme1/liuyundong/xianyinggunYoloV8/huashangdatasets/labels"  # 要生成的txt文件夹
    labelme2yolov2Seg(jsonfilePath=jsonfilePath, resultDirPath=resultDirPath, classList=["0", "1", "2", "3", "4"])
